...
internal sealed class TransmisorSsh : ITransmisor
{
    private readonly Func<ConexionDestino, IClienteTransmision> _fabricaCliente;

    public TransmisorSsh()
        : this(destino => new ScpClienteAdaptado(destino))
    {
    }

    internal TransmisorSsh(Func<ConexionDestino, IClienteTransmision> fabricaCliente)
    {
    ...
    public bool Enviar(IEnumerable<string> rutasLocales, ConexionDestino destino)
    {
        ...
        {
            Console.Error.WriteLine($"[Grupo3][SSH] Fallo al transmitir al destino '{destino.Destino.Id}': {ex.Message}");
            return false;
        }
    }
}
